...
- **Fecha**: 2026-08-20.
- **Tests**: 374 verdes, 30 archivos, 0 fallos.
...
- **Checkpoint Pantalla 5 (Equipos, ver secciones 18/22)**: walkthrough manual completo en local — Login NOVARA → Intake → Productos → Procesos (Elaboración/Envasado/Codificado + etapa 100% custom "Pesada") → Equipos: alta rápida de 6 equipos across 4 grupos (incluyendo bajo la etapa no soportada "Pesada"), editar nombre inline (Llenadora 1 → Llenadora automática), eliminar un equipo sin afectar al resto, persistencia confirmada Atrás→Procesos→Continuar, Guardian sosteniendo la placa "NOVARA" verificado en el DOM, navegación a Capacidades (sin rediseñar, equipo listado, cero capacidad inventada). Verificado sin overflow horizontal en 1440×900 y 1366×768; overflow vertical ~0px en 1440×900 (caso denso: 4 grupos × 6 equipos) y moderado/aceptado en 1366×768, mismo criterio que Pantalla 4. Smoke test de producción repetido tras el deploy: Login → demo → Intake → Productos → Procesos → Equipos, alta de "Reactor 1" confirmada en vivo en `https://guardian-codercup.vercel.app`.
- **Commit actual**: `a5481307aa6acb881dc6c5cb86d56cdbb9ccf8ce` (branch `master`), commiteado y pusheado a `origin/master`.
- **Deploy actual**: verificado directamente contra la API de Vercel (no asumido) — proyecto `guardian-codercup`, deployment `dpl_4ASwxEEZm89U9JUeau3oFfrvYs9d`, commit `a5481307aa6acb881dc6c5cb86d56cdbb9ccf8ce`, status `READY`, `target: production`, servido en `https://guardian-codercup.vercel.app`. Auto-deploy vía Git integration confirmado (push a `master` → deployment automático).
