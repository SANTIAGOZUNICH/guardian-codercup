...
- **Deploy actual**: verificado directamente contra la API de Vercel (no asumido) — proyecto `guardian-codercup`, deployment `dpl_FhYyP3aBrzsWDuALWEJaG9VwVX2C`, commit `54e52fd7c50befee382ba0a07fd00a7ad7d39f22`, status `READY`, `target: production`, servido en `https://guardian-codercup.vercel.app`.
...
- **Checkpoint Pantalla 12 (Command Center)**: walkthrough local NOVARA desde ModelBuilding con 1 producto, 2 procesos (Mezcla especial custom), 1 equipo, capacidad UNKNOWN, personal UNKNOWN, Lun–Vie 08:00–17:00 y Materials SKIP. El home mostró sólo hechos reales: NOVARA dinámico, personal “No especificado”, Materials “No evaluados (opcional)”, proceso custom declarado/aún no simulable y empty state “Todavía no hiciste simulaciones”; no mostró KPIs, constraints, historial ni timestamps ficticios. La consulta “¿Cuál es mi cuello de botella?” se trasladó exacta al input del Ask Guardian existente, sin autoejecutarse ni duplicar parser/chat. Guardian oficial quedó discreto frente al modelo. Acciones verificadas: consultar, explorar modelo y navegación única de `AppShell`; no hay dead buttons decorativos. Cero errores críticos de consola y cero overflow horizontal en 1440×900 y 1366×768. Inmutabilidad del `OperationalModel` cubierta por prueba dirigida. TypeScript limpio, 404 tests/34 archivos, lint y build verdes; el primer build falló sólo por bloqueo de red al descargar Geist y el retry aislado con acceso de red compiló correctamente.
