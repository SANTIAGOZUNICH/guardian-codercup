...
- **NOVICE**: responde bloque por bloque — Productos → Procesos (Pantalla 4, ver abajo) → Equipos → Capacidades (+ variantes por producto, progressive disclosure) → Tiempos de tanda → Personal → Horario → Materiales (opcional).
...
Vitest, 374 tests en 30 archivos, todos deterministas (sin red — ningún test llama a Gemini real). Cobertura por capa: dominio (`presentation.test.ts`), motor (`evaluate-scenario.test.ts` incluye `resolveEffectiveRate` con los 4 niveles de especificidad + el caso crítico anti-escalado), Guided Setup (`guided-setup-v2.test.ts`, `buildModelInputsFromGuidedSetupV2.test.ts` con casos end-to-end de rates por producto), NLU (`presentation-parser.test.ts`, `fixtures.test.ts`), Ask Guardian (`operational-query.test.ts`). Antes de cerrar cualquier checkpoint: `npx tsc --noEmit` + `npx vitest run` + `npm run lint` + `npm run build`, los 4 en verde, más un walkthrough manual real de los flujos afectados (Browser pane, nunca asumido).
...
- **DONE**: Modelo de dominio (gramos/presentaciones), Motor (masa/batches/rates incluyendo precisión por producto-presentación-recurso), Guided Setup (bloques + progressive disclosure + modo avanzado), Ask Guardian (4 categorías de routing), Idioma (100% español), Branding (sin Genus visible), Production References por producto/presentación — **GUARDIAN V1 FUNCTIONAL FREEZE**. Visual Polish — Login/First Impression + Guardian Character Integration, Guided Setup → Productos, Intake "Contanos sobre tu laboratorio" + personalización global de Guardian, Guided Setup → Procesos/Flujo operativo, y Guided Setup → Equipos (ver secciones 18/22): checkpoints visuales cerrados hasta ahora. Flujo de entrada reestructurado: `Login → Intake → Productos → Procesos → Equipos → resto de Guided Setup` (Onboarding/EntryChoice/UploadScreen standalone eliminados, ver sección 18; "Contenido por unidad" ya no es un step obligatorio, ver sección 18). Materials Simulation Rule (sección 10): ausencia de datos de materiales nunca aparece como restricción en el resultado principal — corregido en `simulation-view-model.ts`, `PlanCard.tsx`, `BaselineCard.tsx`, `CommandCenter.tsx`.
- **NEXT**: Visual Polish / Demo Experience para el resto de las pantallas — Guided Setup (capacities, batchTimes, staffing, schedule, materials, review), Building Twin, Command Center, Simulación, Disruption. Un checkpoint por pantalla, extendiendo la dirección visual establecida (secciones 18/22), no redefiniéndola.
...
- **Fecha**: 2026-08-20.
- **Tests**: 366 verdes, 30 archivos, 0 fallos.
...
- **Checkpoint Pantalla 4 (Procesos) + Materials Simulation Rule (ver secciones 10/18/22)**: walkthrough manual completo en local — Login NOVARA → Intake → Productos → Procesos (agregar/quitar/reordenar etapas, verificado sin overflow horizontal en 1440×900 y 1366×768) → Equipos → Capacidades/Tiempos/Personal/Horario/Materiales (todos omitidos u omisibles sin bloquear) → Twin construido → Ask Guardian: simulación sin gramaje conocido → Guardian pregunta gramos antes de simular (nunca corre con datos incompletos) → con 200 g confirmado, simula y muestra resultado (`infeasible` por capacidad sin asignar, dato esperado al omitir Capacidades) SIN mencionar materiales en ningún momento (PlanCard/BaselineCard/Command Center) pese a no haber datos de materiales conectados — confirma la Materials Simulation Rule en vivo. CASO M2/M3 (faltante real / stock suficiente) cubiertos por tests de motor y view-model existentes, no por walkthrough manual (la empresa demo del walkthrough no tenía materiales conectados). Smoke test de producción repetido tras el deploy: Login → demo → Intake → Productos → Procesos, contenido y flujo correctos en `https://guardian-codercup.vercel.app`.
- **Commit actual**: `b632679a7c60837c48d206ca124edc7779ab955d` (branch `master`), commiteado y pusheado a `origin/master`.
- **Deploy actual**: verificado directamente contra la API de Vercel (no asumido) — proyecto `guardian-codercup`, deployment `dpl_3JfVrW1CW1uKZRszBQfWykxgftTN`, commit `b632679a7c60837c48d206ca124edc7779ab955d`, status `READY`, `target: production`, servido en `https://guardian-codercup.vercel.app`. Auto-deploy vía Git integration confirmado (push a `master` → deployment automático).
