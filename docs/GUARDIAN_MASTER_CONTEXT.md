...
- **Deploy actual**: verificado directamente contra la API de Vercel (no asumido) — proyecto `guardian-codercup`, deployment `dpl_58RHPPStvGz9DN4PuxRQTaGT4ULg`, commit `ad6ee510ee6386332c2f2da14be57f479b91fdef`, status `READY`, `target: production`, servido en `https://guardian-codercup.vercel.app`.
- **Checkpoint Pantalla 8 (Días y horarios)**: implementación local validada, todavía sin push/deploy por instrucción explícita. La pantalla muestra el paso real 7/8, Guardian oficial con placa NOVARA, días seleccionables y una única ventana diaria Desde/Hasta. Cero días y rangos inválidos bloquean Continuar con advertencia clara; Atrás vuelve a Personal y conserva los días editados; Continuar navega a Materiales sin rediseñarlo. El calendario confirmado se propaga a constraints, parsing de goals y simulaciones en lugar de `DEFAULT_OPERATIONS_CALENDAR`. Walkthrough local sin overflow en 1440×900 ni 1366×768 y sin errores de consola. TypeScript, 389 tests, lint y build verdes.
