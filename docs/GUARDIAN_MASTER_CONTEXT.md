...
Vitest, 389 tests en 32 archivos, todos deterministas (sin red — ningún test llama a Gemini real). Cobertura por capa: dominio (`presentation.test.ts`), motor (`evaluate-scenario.test.ts` incluye `resolveEffectiveRate` con los 4 niveles de especificidad + el caso crítico anti-escalado), Guided Setup (`guided-setup-v2.test.ts`, `buildModelInputsFromGuidedSetupV2.test.ts` con casos end-to-end de rates por producto y `GuidedSetupScheduleStep.test.ts` para conversión del horario al calendario operacional), NLU (`presentation-parser.test.ts`, `fixtures.test.ts`), Ask Guardian (`operational-query.test.ts`). Antes de cerrar cualquier checkpoint: `npx tsc --noEmit` + `npx vitest run` + `npm run lint` + `npm run build`, los 4 en verde, más un walkthrough manual real de los flujos afectados (Browser pane, nunca asumido).
...
- **DONE**: Modelo de dominio (gramos/presentaciones), Motor (masa/batches/rates incluyendo precisión por producto/presentación/recurso), Guided Setup (bloques + progressive disclosure + modo avanzado), Ask Guardian (4 categorías de routing), Idioma (100% español), Branding (sin Genus visible), Production References por producto/presentación — **GUARDIAN V1 FUNCTIONAL FREEZE**. Visual Polish cerrado para Login/First Impression, Intake y Guided Setup → Productos → Procesos → Equipos → Capacidades → Personal → Días y horarios (ver secciones 9/18/22). Flujo de entrada reestructurado: `Login → Intake → Productos → Procesos → Equipos → Capacidades → Personal → Días y horarios → resto de Guided Setup`; Presentaciones y `batchTimes` no son pantallas obligatorias redundantes, aunque sus datos internos se preservan. Días y horarios representa exactamente el calendario soportado (días activos + una ventana diaria); no muestra descansos ni turnos ficticios y el calendario confirmado alimenta constraints, goals y simulaciones de la sesión. Materials Simulation Rule (sección 10): ausencia de datos de materiales nunca aparece como restricción en el resultado principal. Capacidades adapta la unidad al tipo de proceso y Personal distingue total/breakdown sin double counting ni escalado de capacidad.
- **NEXT**: Visual Polish / Demo Experience para el resto de las pantallas — Guided Setup (materials, review), Building Twin, Command Center, Simulación, Disruption. Un checkpoint por pantalla, extendiendo la dirección visual establecida (secciones 18/22), no redefiniéndola.
...
- **Fecha**: 2026-08-21.
- **Tests**: 389 verdes, 32 archivos, 0 fallos.
