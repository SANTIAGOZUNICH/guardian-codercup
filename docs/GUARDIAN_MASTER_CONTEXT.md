...
**REGLA PERMANENTE — Guardian conserva la identidad del laboratorio (Hotfix Personalización Global, 2026-08-19)**: después del Login, Guardian muestra el `companyName` real en TODAS sus apariciones — mismo personaje, misma personalización; los estados cambian movimiento/pose/luz, nunca la identidad. Fuente global vía React Context (`CompanyNameProvider`/`useCompanyName`, `src/lib/context/CompanyNameContext.tsx`), montado una vez en `GuardianApp.tsx` con `value={session?.companyName ?? null}`. `Guardian` (`GuardianProps.companyName?: string`) resuelve `companyName ?? useCompanyName() ?? undefined` — un valor explícito por prop sigue ganando (override puntual), pero NINGÚN call site necesita pasarlo a mano: Productos y el header compartido de Guided Setup (steps `presentations`/`equipment`/`capacities`/`batchTimes`/`staffing`/`schedule`/`materials`) lo reciben automáticamente por contexto, cero cambio de código en esos JSX. Efecto visual solo en `variant="asset"` (Login no lo muestra — `companyName` todavía no existe al momento de loguearse).

**Pechera corporativa (reemplaza el HUD flotante del Checkpoint 2B, ver Hotfix)**: `GuardianChestPanel` en `Guardian.tsx` — panel matte con gradiente vertical, borde azul extremadamente sutil y bisel (highlight superior + sombra inferior vía `box-shadow` inset) integrado sobre el torso, NO texto flotando encima del PNG. Ancla al CENTRO de la banda real del torso (medida con Pillow: ≈63.6%-74.5% de alto, ≈51% de ancho) y crece simétricamente desde ese punto (`transform: translate(-50%,-50%)`) para que 1 o 2 líneas queden centradas en el mismo lugar. `nameTagFontSize()` calcula el tamaño sobre la PALABRA más larga (no el largo total) — nombres cortos (≤8 caracteres) entran en una línea grande; más largos envuelven a 2 líneas (`line-clamp-2`) en vez de encogerse innecesariamente. Escala proporcionalmente con `size` (padding/radio/fuente derivados de `guardianSize`) — mismo panel en Intake (240px), Productos (220px) y el header de Guided Setup (84px, texto en el piso mínimo de legibilidad ~6.5px, intencionalmente subtil ahí). El nombre exacto ingresado en Login se muestra literal — nunca se antepone "Laboratorio" ni se abrevia.

**Limitación real del asset**: el torso de `guardian-idle.png` es angosto, curvo y glossy (medido con Pillow, banda de contenido opaco útil ≈18-20% del ancho de la imagen) — no una superficie plana tipo remera. La pechera es una pieza integrada por CSS (gradiente + bisel), no un grabado fundido en el chassis. Call sites de estado-dependiente (`alert`/`success` en `ConstraintScreen`/`DisruptionScreen`/`RecommendedPlansScreen`/`ModelBuildingScreen`/`AskGuardianScreen`) y los íconos de marca muy chicos (`CommandCenter` 40px, `AppShell` 30-44px) se dejaron deliberadamente en `variant="svg"` — migrar los primeros perdería la señal de color de alerta/éxito que el chassis SVG anima explícitamente; migrar los segundos degradaría un ícono de marca pequeño a una foto photorealista ilegible a esa escala. Migrarlos es seguro para un checkpoint futuro si se decide llevar también el color de estado al asset 3D.
