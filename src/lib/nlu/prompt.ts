...
export const NLU_SYSTEM_PROMPT = [
  ...
  "- entities.presentations: GUARDIAN V1 usa GRAMOS DE PRODUCTO POR UNIDAD, nunca ml ni densidad. Si el texto menciona cuanto contiene una unidad ('son potes de 200g', 'cada uno lleva 50 gramos', 'el frasco es de 250ml' -> interpretalo igual como gramos, nunca conviertas por densidad), agrega un item con gramsPerUnit y, si se puede determinar, productName (el nombre del producto tal como se menciono). Si el usuario dice explicitamente que no sabe el gramaje ('no se cuanto pesa', 'ni idea'), agrega el item igual con gramsPerUnit: null (nunca lo omitas ni inventes un numero tipico).",
  "- entities.capacityVariants: cuando el texto da una velocidad de un equipo ATADA a un producto o presentacion puntual (ej. 'la primera hace 1800 unidades por hora para frascos de 50g y unas 1000 para 200g', 'Llenadora 1 hace 900 para crema'), agrega un item POR CADA velocidad distinta mencionada, con equipmentName (nombre o referencia del equipo tal como se dijo, ej. 'la primera' o 'Llenadora 1' — null si no se puede determinar a cual equipo se refiere), productName (producto o presentacion, tal como se menciono), value y unit. Nunca uses esto para una velocidad general sin atar a ningun producto — eso va en entities.equipmentV2 (capacityValue) como siempre. Si el equipo no tiene nombre propio pero el texto usa un ordinal ('la primera', 'la segunda') refiriendose a equipos ya mencionados en el mismo mensaje o en 'Contexto: equipos ya conocidos', resolvelo a ese nombre — nunca lo dejes ambiguo si el texto mismo ya lo deja claro.",
  "",
  "Reglas especificas del contexto ask_guardian:",
  "- Si el texto describe un objetivo de produccion Y ademas menciona el gramaje de la presentacion en el mismo mensaje (ej. 'necesito 3000 cremas de 200g para el lunes'), completa TAMBIEN entities.presentations con ese gramaje — no lo dejes solo en el texto libre.",
  "",
