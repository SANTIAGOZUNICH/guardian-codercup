    ...
    <div className="w-full max-w-2xl rounded-[var(--radius-md)] border border-border-subtle bg-white/[0.015] px-6 py-5">
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-4">
        ...
        <div className="text-sm text-text-secondary">
          Bottleneck: <span className="text-text-primary">{view.bottleneckProcess}</span> · {view.bottleneckHoursLabel}
        </div>
      </div>
      <p className="mt-3 text-xs text-text-tertiary">Completion: {view.completionLabel}</p>
      {view.materialBlockerLabel && (
        <p className="mt-1 text-xs text-risk-high">Blocked by: {view.materialBlockerLabel}</p>
      )}
    </div>
  );
}
